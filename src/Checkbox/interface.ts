 ...
 * @en `T = string | number`
 */
export interface CheckboxProps<T extends number | string = any>
  extends Omit<
  ...
  children?:
    | ReactNode
    | ((value: { checked: boolean; indeterminate: boolean }) => ReactNode);
}
...
export interface CheckboxGroupProps<T extends ReactText> {
  ...
  options?: (
    | T
    | { label: ReactNode; value: T; disabled?: boolean; icon?: ReactNode }
  )[];
  /**
   * @zh 选中的选项（受控模式）
   * @en To set value
